...
def load_normalization_statistics(
    ...
    """Load and validate feature-wise training normalization statistics."""
    ...
    with np.load(normalization_path) as normalization_file:
        available_keys = set(normalization_file.files)
        ...
        standard_deviation_key_candidates = (
            "standard_deviation",
            "std",
        ...
        standard_deviation_key = next(
            (
                key
                for key in standard_deviation_key_candidates
                if key in available_keys
            ),
            None
        )

        if mean_key is None or standard_deviation_key is None:
            raise KeyError(
                "Unable to identify the normalization arrays. "
        ...
        standard_deviation = cast(
            Float32Array,
            np.asarray(
                normalization_file[standard_deviation_key],
                dtype=np.float32
            )
        )
    ...
    if mean.shape != standard_deviation.shape:
        raise ValueError(
            "Normalization mean and standard deviation have "
            "different shapes."
        )

    if mean.size != 700:
        raise ValueError(
            "Unexpected normalization trace length: "
            f"expected 700, found {mean.size}."
        )

    if not bool(np.isfinite(mean).all()):
        raise ValueError(
            "The normalization mean contains NaN or infinite values."
        )

    if not bool(np.isfinite(standard_deviation).all()):
        raise ValueError(
            "The normalization standard deviation contains NaN "
            "or infinite values."
        )

    if bool(np.any(standard_deviation <= 0.0)):
        raise ValueError(
            "Every normalization standard deviation must be positive."
        )
...
class ASCADAttackDataset(Dataset[Tensor]):
    """Load and normalize ASCAD attack traces."""
    ...
    def __init__(
        ...
        if not h5_path.is_file():
            raise FileNotFoundError(
                f"ASCAD dataset not found: {h5_path}"
            )
        ...
        with h5py.File(self.h5_path, "r") as h5_file:
            ...
            total_traces = int(
                traces_dataset.shape[0]
            )

            trace_length = int(
                traces_dataset.shape[1]
            )

        if trace_length != self.mean.size:
            raise ValueError(
                "Attack trace length does not match the normalization "
                f"statistics: traces={trace_length}, "
                f"normalization={self.mean.size}."
            )
    ...
    def _open_file(self) -> None:
        """Open the HDF5 file lazily inside the active process."""
    ...
    def __getitem__(self, index: int) -> Tensor:
        ...
        if self._traces_dataset is None:
            raise RuntimeError(
                "Unable to open the ASCAD attack traces."
            )
        ...
        normalized_trace = cast(
            Float32Array,
            (trace - self.mean) / self.standard_deviation
        )

        if not bool(np.isfinite(normalized_trace).all()):
            raise FloatingPointError(
                f"Normalized attack trace {index} contains NaN "
                "or infinite values."
            )

        normalized_trace = np.expand_dims(
            normalized_trace,
            axis=0
        )
    ...
    def close(self) -> None:
        """Close the lazily opened HDF5 file."""
        if self._h5_file is not None:
            self._h5_file.close()
            self._h5_file = None
...
def load_model(
    ...
    """Load a trained CNN-Mamba-MLP checkpoint."""
...
def save_guessing_entropy_plot(
    ...
    plt.title("CNN-Mamba-MLP ASCAD - Guessing Entropy")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
...
def save_success_rate_plot(
    ...
    plt.title("CNN-Mamba-MLP ASCAD - Success Rate")
    plt.ylim(-0.02, 1.02)
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate a trained CNN-Mamba-MLP model on the ASCAD attack traces."
        )
    )
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "checkpoints/cnn_mamba_mlp_ascad/phase_1/best_model.pt"
        ),
        help="Trained CNN-Mamba-MLP checkpoint."
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Feature-wise normalization statistics computed exclusively "
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("results/cnn_mamba_mlp_ascad/phase_1"),
...
def main() -> None:
    ...
    print("=" * 70)
    print("CNN-MAMBA-MLP ASCAD ATTACK EVALUATION")
    print("=" * 70)
    ...
    print("Preprocessing:       training-split feature-wise normalization")
    print("=" * 70)

    start_time = time.perf_counter()
    dataset = ASCADAttackDataset(
