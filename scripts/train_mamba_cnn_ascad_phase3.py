...
DEFAULT_PHASE_TWO_CHECKPOINT = (
    PROJECT_ROOT
    / "checkpoints/mamba_cnn_ascad/phase_2/best_model.pt"
)

DEFAULT_CHECKPOINT_DIRECTORY = (
    PROJECT_ROOT
    / "checkpoints/mamba_cnn_ascad/phase_3"
)
...
def parse_arguments() -> TrainingConfig:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run phase-three Mamba-CNN fine-tuning by training the Mamba "
            "front-end and Conv1-Conv5 while keeping the classifier frozen."
        )
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_PHASE_TWO_CHECKPOINT,
        help="Best phase-two checkpoint used to initialize phase three."
    )
    ...
    config = TrainingConfig(
        ...
        phase_two_checkpoint=arguments.phase_two_checkpoint,
...
def validate_config(config: TrainingConfig) -> None:
    ...
    if config.deep_cnn_learning_rate <= 0.0:
        raise ValueError(
            "--deep-cnn-learning-rate must be greater than zero."
        )
...
def load_phase_two_checkpoint(
    model: MambaCnnAscad,
    checkpoint_path: Path
) -> None:
    """Load the best phase-two model weights."""
    checkpoint_path = checkpoint_path.resolve()

    if not checkpoint_path.is_file():
        raise FileNotFoundError(
            f"Phase-two checkpoint not found: {checkpoint_path}"
        )
...
def configure_phase_three_trainable_components(
    model: MambaCnnAscad
) -> None:
    """
    Configure the phase-three trainable and frozen components.
    ...
    if any(
        ...
        raise RuntimeError(
            "The classifier must remain frozen during phase three."
        )
...
def collect_parameter_groups(
    ...
    """Create the Adam parameter groups used during phase three."""
...
def train_one_epoch(
    ...
    """Train the phase-three components for one epoch."""
    model.train()
...
def config_to_serializable_dict(
    ...
    config_dictionary["phase_two_checkpoint"] = str(
        config.phase_two_checkpoint.resolve()
    )
...
def save_checkpoint(
    ...
    *,
    ...
    """Save the complete phase-three training state."""
...
def save_training_history(
    ...
    """Save phase-three training metrics in JSON format."""
...
def load_training_history(
    ...
    """Load a previously saved phase-three training history."""
...
def resume_training(
    ...
    if training_phase_object != 3:
        raise ValueError(
            "The resume checkpoint is not a phase-three checkpoint."
        )
...
def main() -> None:
    """Run phase-three fine-tuning for the Mamba-CNN model."""
    ...
    print()
    print("Mamba-CNN ASCAD phase-three training")
    print("------------------------------------")
    ...
    print(
        "Phase-two checkpoint:      "
        f"{config.phase_two_checkpoint.resolve()}"
    )
    ...
    load_phase_two_checkpoint(
        model=model,
        checkpoint_path=config.phase_two_checkpoint
    )

    configure_phase_three_trainable_components(
        model
    )
    ...
    print()
    print("Phase-three training completed")
    print("------------------------------")
    print(
        "Total duration:       "
