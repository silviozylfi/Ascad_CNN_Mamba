...
DEFAULT_CHECKPOINT_DIRECTORY = (
    PROJECT_ROOT
    / "checkpoints/cnn_mamba_mlp_ascad/phase_1"
)
...
class TrainingConfig:
    """Configuration for phase-one CNN-Mamba-MLP training."""
...
def parse_arguments() -> TrainingConfig:
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-4,
        help="Learning rate for Mamba, MLP, LayerNorm and classifier FC1."
    )

    parser.add_argument(
        "--transferred-convolutions-learning-rate",
        type=float,
        default=1e-5,
        help="Learning rate for the transferred Conv1 through Conv5 layers."
    )

    parser.add_argument(
        "--transferred-classifier-learning-rate",
        type=float,
        default=1e-5,
        help="Learning rate for the transferred FC2 and prediction layers."
    )
    ...
    config = TrainingConfig(
        ...
        transferred_convolutions_learning_rate=(
            arguments.transferred_convolutions_learning_rate
        ),
        transferred_classifier_learning_rate=(
            arguments.transferred_classifier_learning_rate
        ),
...
def validate_config(config: TrainingConfig) -> None:
    """Validate the phase-one training configuration."""
    if config.epochs <= 0:
    ...
    if config.transferred_convolutions_learning_rate <= 0.0:
        raise ValueError(
            "--transferred-convolutions-learning-rate must be greater than zero."
        )

    if config.transferred_classifier_learning_rate <= 0.0:
        raise ValueError(
            "--transferred-classifier-learning-rate must be greater than zero."
        )
...
def load_initialization_checkpoint(
    ...
    """Load the partially pretrained initialization checkpoint."""
...
def collect_optimizer_parameter_groups(
    ...
    """Create the three optimizer groups used in revised phase one."""
...
def train_one_epoch(
    ...
    """Train phase-one components for one epoch."""
    model.train()
...
def resume_training(
    ...
    """Restore model, optimizer and phase-one progress."""
    checkpoint_path = checkpoint_path.resolve()
    ...
    if checkpoint_mapping.get("training_phase") != 1:
        raise ValueError(
            "The resume checkpoint is not a phase-one checkpoint."
        )
...
def main() -> None:
    """Run phase-one CNN-Mamba-MLP training."""
    ...
    print()
    print("CNN-Mamba-MLP ASCAD phase-one training")
    print("--------------------------------------")
    ...
    print(
        "Transferred convolutions LR:"
        f" {config.transferred_convolutions_learning_rate}"
    )
    print(
        "Transferred classifier LR: "
