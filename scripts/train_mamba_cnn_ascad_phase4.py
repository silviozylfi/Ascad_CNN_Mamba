...
DEFAULT_PHASE_THREE_CHECKPOINT = (
    PROJECT_ROOT
    / "checkpoints/mamba_cnn_ascad/phase_3/best_model.pt"
)

DEFAULT_CHECKPOINT_DIRECTORY = (
    PROJECT_ROOT
    / "checkpoints/mamba_cnn_ascad/phase_4"
)

NUM_CLASSES = 256

FRONT_END_GROUP_NAME = "front_end"
MIDDLE_CNN_GROUP_NAME = "conv2_conv3"
DEEP_CNN_GROUP_NAME = "conv4_conv5"
CLASSIFIER_GROUP_NAME = "classifier"
...
class TrainingConfig:
    ...
    middle_cnn_learning_rate: float
    deep_cnn_learning_rate: float
    classifier_learning_rate: float
    gradient_clip_norm: float
    num_workers: int
    random_seed: int
    patience: int
    phase_three_checkpoint: Path
...
def parse_arguments() -> TrainingConfig:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run phase-four Mamba-CNN fine-tuning by training the entire network "
    ...
    parser.add_argument(
        "--deep-cnn-learning-rate",
        type=float,
        default=5e-7,
        help="Learning rate for Conv4 and Conv5."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-7,
        help="Learning rate for the complete classifier."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_PHASE_THREE_CHECKPOINT,
        help="Best phase-three checkpoint used to initialize phase four."
    )
    ...
    config = TrainingConfig(
        ...
        middle_cnn_learning_rate=arguments.middle_cnn_learning_rate,
        deep_cnn_learning_rate=arguments.deep_cnn_learning_rate,
        ...
        phase_three_checkpoint=arguments.phase_three_checkpoint,
...
def validate_config(config: TrainingConfig) -> None:
    ...
    if config.middle_cnn_learning_rate <= 0.0:
        raise ValueError(
            "--middle-cnn-learning-rate must be greater than zero."
        )
...
def load_phase_three_checkpoint(
    model: MambaCnnAscad,
    checkpoint_path: Path
) -> None:
    """Load the best phase-three model weights."""
    checkpoint_path = checkpoint_path.resolve()

    if not checkpoint_path.is_file():
        raise FileNotFoundError(
            f"Phase-three checkpoint not found: {checkpoint_path}"
        )
...
def configure_phase_four_trainable_components(
    model: MambaCnnAscad
) -> None:
    """
    ...
    if not all(
        ...
        raise RuntimeError(
            "Every model parameter must be trainable during phase four."
        )
...
def collect_parameter_groups(
    ...
    """Create the Adam parameter groups used during phase four."""
    ...
    conv4 = get_convolution(
        model=model,
        layer_index=9,
        layer_name="Conv4"
    )

    conv5 = get_convolution(
        model=model,
        layer_index=12,
        layer_name="Conv5"
    )
    ...
    parameter_groups: list[dict[str, Any]] = [
        ...
        {
            "params": middle_cnn_parameters,
            "lr": config.middle_cnn_learning_rate,
            "group_name": MIDDLE_CNN_GROUP_NAME
        },
        {
            "params": deep_cnn_parameters,
            "lr": config.deep_cnn_learning_rate,
            "group_name": DEEP_CNN_GROUP_NAME
        },
        {
            "params": classifier_parameters,
            "lr": config.classifier_learning_rate,
            "group_name": CLASSIFIER_GROUP_NAME
        }
    ]
...
def print_optimizer_groups(
    ...
    """Print the learning rate and parameter count of each optimizer group."""
    print()
...
def train_one_epoch(
    ...
    """Train all model components for one phase-four epoch."""
    model.train()
...
def config_to_serializable_dict(
    ...
    config_dictionary["phase_three_checkpoint"] = str(
        config.phase_three_checkpoint.resolve()
    )
...
def save_checkpoint(
    ...
    *,
    ...
    """Save the complete phase-four training state."""
...
def save_training_history(
    ...
    """Save phase-four training metrics in JSON format."""
...
def load_training_history(
    ...
    """Load a previously saved phase-four training history."""
...
def resume_training(
    ...
    if training_phase_object != 4:
        raise ValueError(
            "The resume checkpoint is not a phase-four checkpoint."
        )
...
def main() -> None:
    """Run phase-four fine-tuning for the Mamba-CNN model."""
    ...
    print()
    print("Mamba-CNN ASCAD phase-four training")
    print("------------------------------------")
    ...
    print(
        ...
        f"{config.middle_cnn_learning_rate}"
    )
    print(
        "Conv4-Conv5 learning rate: "
        f"{config.deep_cnn_learning_rate}"
    )
    print(
        "Classifier learning rate:  "
        f"{config.classifier_learning_rate}"
    )
    print(
        "Gradient clip norm:        "
    ...
    print(
        "Phase-three checkpoint:    "
        f"{config.phase_three_checkpoint.resolve()}"
    )
    ...
    load_phase_three_checkpoint(
        model=model,
        checkpoint_path=config.phase_three_checkpoint
    )

    configure_phase_four_trainable_components(
        model
    )
    ...
    print()
    print("Phase-four training completed")
    print("------------------------------")
    print(
        "Total duration:       "
