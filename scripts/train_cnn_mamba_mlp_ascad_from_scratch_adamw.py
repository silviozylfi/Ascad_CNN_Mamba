...
DEFAULT_CHECKPOINT_DIRECTORY = (
    PROJECT_ROOT
    / "checkpoints/cnn_mamba_mlp_ascad/from_scratch_adamw"
)
...
class TrainingConfig:
    """Configuration for the AdamW from-scratch experiment."""

    epochs: int
    batch_size: int
    new_layers_learning_rate: float
    transferred_convolutions_learning_rate: float
    transferred_classifier_learning_rate: float
    weight_decay: float
...
def parse_arguments() -> TrainingConfig:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Train the newly introduced CNN-Mamba-MLP components while "
            "keeping the transferred ASCAD convolutions and final classifier "
            "layers frozen."
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Maximum total number of phase-one epochs."
    )
    ...
    parser.add_argument(
        "--new-layers-learning-rate",
        type=float,
        default=1e-4,
        help="Learning rate for BatchNorm, Mamba, MLP, LayerNorm and classifier FC1."
    )

    parser.add_argument(
        "--convolutions-learning-rate",
        type=float,
        default=1e-4,
        help="Learning rate for Conv1 through Conv5."
    )

    parser.add_argument(
        "--classifier-tail-learning-rate",
        type=float,
        default=1e-4,
        help="Learning rate for classifier FC2 and the prediction layer."
    )
    ...
    config = TrainingConfig(
        ...
        new_layers_learning_rate=arguments.new_layers_learning_rate,
        transferred_convolutions_learning_rate=(
            arguments.convolutions_learning_rate
        ),
        transferred_classifier_learning_rate=(
            arguments.classifier_tail_learning_rate
        ),
...
def validate_config(config: TrainingConfig) -> None:
    """Validate the AdamW from-scratch training configuration."""
    if config.epochs <= 0:
    ...
    if config.new_layers_learning_rate <= 0.0:
        raise ValueError(
            "--new-layers-learning-rate must be greater than zero."
        )

    if config.transferred_convolutions_learning_rate <= 0.0:
        raise ValueError(
            "--convolutions-learning-rate must be greater than zero."
        )

    if config.transferred_classifier_learning_rate <= 0.0:
        raise ValueError(
            "--classifier-tail-learning-rate must be greater than zero."
        )
    ...
    if not 0.0 <= config.label_smoothing < 1.0:
        raise ValueError(
            "--label-smoothing must be in the interval [0, 1)."
        )
...
def configure_phase_one_trainable_components(
    model: CnnMambaMlpAscad
) -> None:
    """
    Configure every model component as trainable.

    The transferred convolutions and transferred classifier layers use
    smaller learning rates than the newly introduced components.
    """
    for parameter in model.parameters():
        parameter.requires_grad = True


def collect_optimizer_parameter_groups(
    model: CnnMambaMlpAscad,
    config: TrainingConfig
) -> tuple[list[dict[str, Any]], list[nn.Parameter]]:
    """Create optimizer groups for the from-scratch experiment."""
    classifier_normalization = model.classifier[0]
    classifier_fc1 = model.classifier[1]
    classifier_fc2 = model.classifier[3]
    classifier_predictions = model.classifier[5]

    if not isinstance(classifier_normalization, nn.LayerNorm):
        raise TypeError(
            "Classifier layer 0 must be nn.LayerNorm."
        )

    if not isinstance(classifier_fc1, nn.Linear):
        raise TypeError(
            "Classifier layer 1 must be nn.Linear."
        )

    if not isinstance(classifier_fc2, nn.Linear):
        raise TypeError(
            "Classifier layer 3 must be nn.Linear."
        )

    if not isinstance(classifier_predictions, nn.Linear):
        raise TypeError(
            "Classifier layer 5 must be nn.Linear."
        )

    transferred_convolution_parameters: list[nn.Parameter] = []
    batch_norm_parameters: list[nn.Parameter] = []

    for module in model.feature_extractor.modules():
        if isinstance(module, nn.Conv1d):
            transferred_convolution_parameters.extend(
                module.parameters()
            )
        elif isinstance(module, nn.BatchNorm1d):
            batch_norm_parameters.extend(
                module.parameters()
            )
    ...
    new_layer_parameters = [
        ...
        *classifier_normalization.parameters(),
        *classifier_fc1.parameters()
    ]

    transferred_classifier_parameters = [
        *classifier_fc2.parameters(),
        *classifier_predictions.parameters()
    ]
    ...
    grouped_parameter_ids = {
        ...
        for parameter in (
            new_layer_parameters
            + transferred_convolution_parameters
            + transferred_classifier_parameters
        )
    }
    ...
    if grouped_parameter_ids != trainable_parameter_ids:
        raise RuntimeError(
            "Optimizer groups do not match all trainable parameters."
        )

    parameter_groups: list[dict[str, Any]] = [
        {
            "params": new_layer_parameters,
            "lr": config.new_layers_learning_rate,
            "group_name": "new_layers"
        },
        {
            "params": transferred_convolution_parameters,
            "lr": config.transferred_convolutions_learning_rate,
            "group_name": "convolutions"
        },
        {
            "params": transferred_classifier_parameters,
            "lr": config.transferred_classifier_learning_rate,
            "group_name": "classifier_tail"
        }
    ]
...
def train_one_epoch(
    ...
    """Train the complete randomly initialized model for one epoch."""
    model.train()
    ...
    for traces, labels in dataloader:
        ...
        correct_predictions, sample_count = compute_batch_accuracy(
            logits=logits,
            labels=labels
        )
...
def save_checkpoint(
    ...
    *,
    ...
    """Save the complete AdamW from-scratch training state."""
    ...
    checkpoint = {
        "training_experiment": "from_scratch_adamw",
        "epoch": epoch,
...
def save_training_history(
    ...
    """Save from-scratch training metrics in JSON format."""
...
def load_training_history(
    ...
    """Load a previously saved from-scratch training history."""
    ...
    with history_path.open(
        ...
        loaded_history: object = json.load(
            history_file
        )
...
def move_optimizer_state_to_device(
    ...
    """Move restored optimizer tensors to the selected device."""
    for optimizer_state in optimizer.state.values():
        for state_name, state_value in optimizer_state.items():
            if isinstance(state_value, Tensor):
                optimizer_state[state_name] = state_value.to(
                    device
                )
...
def resume_training(
    ...
    """Restore model, optimizer and from-scratch training progress."""
    ...
    if checkpoint_mapping.get("training_experiment") != "from_scratch_adamw":
        raise ValueError(
            "The resume checkpoint is not a from-scratch AdamW checkpoint."
        )

    model_state = extract_state_dict(
        loaded_checkpoint
    )
...
def main() -> None:
    """Train CNN-Mamba-MLP from scratch with AdamW."""
    ...
    print()
    print("CNN-Mamba-MLP ASCAD training from scratch with AdamW")
    print("----------------------------------------------------")
    ...
    print(
        "New layers learning rate:  "
        f"{config.new_layers_learning_rate}"
    )
    print(
        "Convolutions learning rate: "
        f" {config.transferred_convolutions_learning_rate}"
    )
    print(
        "Classifier tail LR:        "
        f"{config.transferred_classifier_learning_rate}"
    )
    ...
    configure_phase_one_trainable_components(
        model
    )
    ...
    parameter_groups, trainable_parameters = (
        collect_optimizer_parameter_groups(
            model=model,
            config=config
        )
    )
    ...
    for epoch in range(
        ...
        epoch_metrics: dict[str, float | int] = {
            ...
            "convolutions_learning_rate": float(
                optimizer.param_groups[1]["lr"]
            ),
            "classifier_tail_learning_rate": float(
    ...
    print()
    print("From-scratch training completed")
    print("-------------------------------")
    print(
        "Total duration:       "
